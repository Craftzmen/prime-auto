...
const criteria = [
  { icon: UserCheck, title: "Identity verification", text: "Basic identity and contact information help align the file with the correct applicant." },
  { icon: FileInput, title: "Income or affordability review", text: "We review the payment profile so the monthly structure is realistic." },
  { icon: BadgeCheck, title: "Vehicle context", text: "New, used, and refinance requests are handled with different underwriting assumptions." },
  { icon: Wallet, title: "Budget alignment", text: "The final recommendation should fit the payment plan, not just the sticker price." },
];
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[1fr_0.95fr] gap-10 lg:gap-14 items-center">
          <div>
            <motion.div initial={{ opacity: 0, y: 12 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="mb-6 inline-flex items-center gap-3 bg-white/[0.03] border border-white/[0.08] px-4 py-2 rounded-full">
              ...
              <span className="text-[11px] font-bold text-white uppercase tracking-[0.2em]">Eligibility</span>
            </motion.div>
            <motion.h2 initial={{ opacity: 0, y: 14 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: 0.06 }} className="text-4xl md:text-5xl font-bold text-white tracking-tight leading-[1.1]">
              Applications reviewed against a <span className="text-gradient-primary">practical affordability model</span>
            </motion.h2>
            <motion.p initial={{ opacity: 0, y: 14 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: 0.1 }} className="text-base text-white/45 leading-relaxed mt-4 max-w-xl">
              The objective is not to overcomplicate the file. It is to make the requirements visible so customers can prepare the right documents once.
            </motion.p>
