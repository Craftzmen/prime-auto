...
import { BadgeCheck, ClipboardList, Gauge, Timer } from "lucide-react";
import { siteImages } from "@/lib/site-images";

const checks = [
  { icon: BadgeCheck, title: "Repair verification", text: "The completed job is checked against the original complaint and the documented fix." },
  { icon: ClipboardList, title: "Final checklist", text: "Fluid levels, fasteners, warning lights, and test outcomes are verified before delivery." },
  { icon: Gauge, title: "Road-ready validation", text: "We confirm the vehicle behaves correctly under real operating conditions." },
  { icon: Timer, title: "Timely handoff", text: "The customer gets a realistic completion window rather than an optimistic guess." },
];
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[0.95fr_1.05fr] gap-10 lg:gap-14 items-center">
          <motion.div initial={{ opacity: 0, y: 18 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="relative h-[300px] lg:h-[460px] rounded-[2rem] overflow-hidden border border-white/[0.08] order-2 lg:order-1 glass-card p-2">
            <div className="relative w-full h-full rounded-3xl overflow-hidden">
              <Image src={siteImages.services.assurance} alt="Quality assurance in an automotive shop" fill className="object-cover object-center opacity-80" />
          ...
          <div className="order-1 lg:order-2">
            <motion.div initial={{ opacity: 0, y: 12 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="mb-6 inline-flex items-center gap-3 bg-white/[0.03] border border-white/[0.08] px-4 py-2 rounded-full">
              ...
              <span className="text-[11px] font-bold text-white uppercase tracking-[0.2em]">Quality Assurance</span>
            </motion.div>
            <motion.h2 initial={{ opacity: 0, y: 14 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: 0.06 }} className="text-4xl md:text-5xl font-bold text-white tracking-tight leading-[1.1]">
              Every service finishes with a <span className="text-gradient-primary">technical review</span>
            </motion.h2>
            <motion.p initial={{ opacity: 0, y: 14 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: 0.1 }} className="text-base text-white/45 leading-relaxed mt-4 max-w-xl">
              We do not treat completion as the last wrench turn. A job is finished when the result is verified and the customer can understand the outcome.
            </motion.p>
